...
<!--                </div>-->
        ...
        <el-dialog
            title="复制API"
            :visible.sync="apiCopyDialogVisible"
            width="30%"
            >
            <el-form>
                <el-form-item label="项目列表" label-width="100px">
                    <el-select v-model="form.projectId" placeholder="将API复制到对应项目" @change="changeProjectChoice">                 
                        <el-option            
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="handleAPICopy">确 定</el-button>
            </span>
        </el-dialog>
    </el-container>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeProjectChoice(){
                this.$api.getTree(this.form.projectId, {params: {type: 1}}).then(resp => {
                    this.form.dataTree = resp['tree']
                    })
            },
            handleAPICopy(){
                this.$api.copyAPI(this.form.apiId, {
                        'name': this.form.name,
                        ...
                        if (resp.success) {
                            ...
                            this.$message({
                                type: "success",
                                message: "复制API成功，一共复制"+this.form.treeId.length + "条"
                            });
                        } else {
                            this.$message.error(resp.msg);
                        }
