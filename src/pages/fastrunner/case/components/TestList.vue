<template>
    <el-container>
            ...
            <div style="overflow: hidden">
                <el-row :gutter="50">
                    <el-col :span="8" v-if="testData.count >= 0">
                        ...
                        <el-button type="primary" style="display:inline-block" @click="debug()">查看运行结果</el-button>
        ...
        <el-dialog
            title="复制用例集名称"
            :visible.sync="suiteCopyDialogVisible"
            width="30%"
            >
            <el-form>
                <el-form-item label="项目列表" label-width="100px">
                    <el-select v-model="form.projectId" placeholder="将用例集复制到对应项目" @change="changeProjectChoice">                 
                        <el-option            
                ...
                <el-form-item label="模块目录" label-width="100px">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="handleSuiteCopy">确 定</el-button>
            </span>
        </el-dialog>
    </el-container>
...
export default {
    ...
    methods: {
        currentChecked (nodeObj, SelectedObj) {
                ...
                this.form.treeId = SelectedObj.checkedKeys
            },
        
        changeProjectChoice(){
                this.$api.getTree(this.form.projectId, {params: {type: 2}}).then(resp => {
                    this.form.dataTree = resp['tree']
                    })
            },
        handleSuiteCopy(){
            this.$api.copySuite(this.form.suiteId, {
                        'name': this.form.name,
                        'treeId':this.form.treeId,
                        'projectId':this.form.projectId
                    }).then(resp => {
                        if (resp.success) {
                            this.suiteCopyDialogVisible = false
                            this.getTestList();
                            this.$message({
                                type: "success",
                                message: "复制SUITE成功，一共复制"+this.form.treeId.length + "条"
                            });
                        } else {
        ...
        * @param name，用例名称，测试报告使用这个名称
        ...
        handleCopyTest(id, name) {
            ...
            this.suiteCopyDialogVisible = true
            this.form.suiteId = id
            this.form.name = name
        },
